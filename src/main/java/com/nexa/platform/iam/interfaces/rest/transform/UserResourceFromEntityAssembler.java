...
public final class UserResourceFromEntityAssembler {
    ...
    public static UserResource toResourceFromEntity(UserResponse response) {
        return new UserResource(response.id(), response.fullName(), response.displayName(), response.email(),
            ...
            response.tenantId(), response.workspaceId(), response.workspaceSlug(), response.workspaceStatus(),
            response.membershipStatus(), response.clientAccountId(), response.phone(),
            response.preferredLanguage(), response.criticalNotificationsEnabled());
    }
}
