...
public final class OrderResourceFromEntityAssembler {
    ...
    public static OrderResource toResourceFromEntity(OrderResponse response) {
        return new OrderResource(response.id(), response.backendId(), response.tenantId(), response.orderNumber(), response.customerId(),
            ...
            response.total(), response.notes(), response.paymentConfirmation(), response.inventoryReservation(),
            response.rejectionReason(), response.confirmedAt(), response.createdAt(), response.updatedAt());
    }
