...
| Sales | `/api/v1/purchase-requests` | Buyer portal purchase request list/detail, alias `/api/v1/requests` |
...
| Invoicing | `/api/v1/documents` | Buyer-visible business documents, alias `/api/v1/business-documents` |
| Invoicing | `/api/v1/payments` | Payment records |
| Shared | `/api/v1/health` | Public health check |

## Security

Swagger/OpenAPI, health, authentication, organization registration POST and tenant preview GET are public. Every other API route requires JWT authentication; tenant/workspace scope is derived from the authenticated membership.
