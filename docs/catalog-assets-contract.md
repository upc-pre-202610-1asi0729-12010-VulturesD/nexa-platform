...
| `brand` | Source-compatible brand alias | Open Source catalog adapters |
| `unitPrice` | Unit price | Order and request summaries |
| `price` | Source-compatible price alias | Open Source catalog adapters |
...
| `catalogItemId`, `productId` | Source-compatible identifiers | Open Source catalog adapters |
| `itemName` | Source-compatible product name | Open Source catalog adapters |
| `categoryName` | Source-compatible category name | Open Source catalog adapters |
| `unitPriceAmount`, `unitPriceCurrency` | Source-compatible price object flattening | Open Source catalog adapters |
...
The Platform stores relative image URLs only. PostgreSQL stores references while media files remain in the WebApp assets.
...
Platform local/test/seed bootstrap reads `src/main/resources/data/catalog-items.json`, copied from the Open Source WebApp source-aligned fallback catalog. It contains 50 products and keeps the same first product as Open Source:
